...
GO
PRINT N'Altering Procedure [dbo].[GetServicioAdquiridoChangesByRowVersion]...';


GO
ALTER  PROCEDURE [dbo].[GetServicioAdquiridoChangesByRowVersion]
(
    @startRow BIGINT 
   ,@endRow  BIGINT 
)
...
BEGIN
 ...
 select ser.Id IdServicio , Count(act.ID) NumeroActividades, STRING_AGG(CONVERT(NVARCHAR(max), ISNULL(act.Nombre,'N/A')), ',')  ListaActividad
 into #ServicioListaActividad
  ...
  ser.id_autobus ,
  ser.id_ruta ,
  SUM (costRuta.TotalCostoTransporte + costAct.TotalCostoActividad)*0.30 PrecioPaquete ,
   SUM (costRuta.TotalCostoTransporte + costAct.TotalCostoActividad) CostoServicio ,
  serAd.id_Turista ,
  ListAct.NumeroActividades ,
  ListAct.ListaActividad 
	FROM [dbo].[ServicioAdquirido] serAd
	INNER JOIN Servicio ser on ser.Id = serAd.id_Servicio
	INNER JOIN #ServicioCostoTransporte costRuta on ser.id = costRuta.IdServicio
	INNER JOIN #ServicioCostoActividad costAct on ser.Id = costAct.IdServicio
	INNER JOIN #ServicioListaActividad ListAct on ser.Id = ListAct.IdServicio
	INNER JOIN Turista tur on serAd.id_Turista = tur.id
